...
* 用户必须继承并实现run方法
* 另外，该类（Thread）继承自类Shared，这样就可以使用智能指针了
*/
...
public:
     ...
     * 
     * 调用系统函数 pthread_create
     */
    int  start(size_t stackSize= 0);

    /** 
     * @brief 
     * 如果底层的线程还没有退出(也就是说，还没有离开它的run方法)，这个方法返回真；否则返回假。
     * 如果你想要实现非阻塞式的join，isAlive 很有用
     */
    bool isAlive() const; 
     ...
     * 如果你针对已经分离的线程、或是已经汇合的线程调用detach，会产生不确定的行为
     * 
     * 调用系统函数 pthread_detach
     */
    int detach();
